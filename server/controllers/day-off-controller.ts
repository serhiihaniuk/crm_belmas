...
class DayOffController {
	static async createDayOff({
		dayCode,
		employeeID
	}: {
		dayCode: DayCode;
		employeeID: string;
	}): Promise<MongoResponse<IDayOffRaw>> {
		...
		try {
			...
			const dayOffCreated = new DayOff(dayOff);
			...
			log.error(
				`${controllerName}createDayOff`,
				`Error creating day off for employee ${employeeID} on ${dayCode}`
			);
			throw new Error(e);
		}
	}

	static async deleteDayOff({dayOffID}: {dayOffID: string}): Promise<MongoResponse<IDayOffRaw>> {
